class Solution:
    def nearestExit(self, maze: List[List[str]], entrance: List[int]) -> int:
        ...
        while q:
            ...
            for i in range(
                stepLength
            ):  # here, we can use length of the deque to iterate through level by level
                node = q.popleft()
                if (
                    node[0] == 0
                ...
                for d in dirs:
                    ...
                    ):  # only add valid nodes to the queue!
                        visited.add(
                            newD
                        )  # mark it as visited, so we don't loop forever
                        q.append(newD)
            step += 1  # after we finish with a level, increment step by one
        return -1


# Time : 642ms (82.25%) Space : 18.25 MB (20.56%)
